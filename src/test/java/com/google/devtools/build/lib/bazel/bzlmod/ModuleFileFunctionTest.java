...
public class ModuleFileFunctionTest extends FoundationTestCase {
  ...
  @Test
  public void testRootModule_moduleImport_good() throws Exception {
    scratch.overwriteFile(
        rootDirectory.getRelative("MODULE.bazel").getPathString(),
        "module(name='aaa')",
        "module_import('//java:MODULE.bazel.segment')",
        "bazel_dep(name='foo', version='1.0')",
        "register_toolchains('//:whatever')",
        "module_import('//python:MODULE.bazel.segment')");
    ...
    scratch.overwriteFile(
        ...
        "module_import('//python:toolchains/MODULE.bazel.segment')");
    scratch.overwriteFile(
  ...
  @Test
  public void testRootModule_moduleImport_bad_otherRepoLabel() throws Exception {
    scratch.overwriteFile(
        rootDirectory.getRelative("MODULE.bazel").getPathString(),
        "module(name='aaa')",
        "module_import('@haha//java:MODULE.bazel.segment')");
    FakeRegistry registry = registryFactory.newFakeRegistry("/foo");
    ModuleFileFunction.REGISTRIES.set(differencer, ImmutableList.of(registry.getUrl()));
  ...
  @Test
  public void testRootModule_moduleImport_bad_relativeLabel() throws Exception {
    scratch.overwriteFile(
        rootDirectory.getRelative("MODULE.bazel").getPathString(),
        "module(name='aaa')",
        "module_import(':MODULE.bazel.segment')");
    FakeRegistry registry = registryFactory.newFakeRegistry("/foo");
    ModuleFileFunction.REGISTRIES.set(differencer, ImmutableList.of(registry.getUrl()));
  ...
  @Test
  public void testRootModule_moduleImport_bad_badLabelSyntax() throws Exception {
    scratch.overwriteFile(
        rootDirectory.getRelative("MODULE.bazel").getPathString(),
        "module(name='aaa')",
        "module_import('//haha/:::')");
    ...
    EvaluationResult<RootModuleFileValue> result =
        evaluator.evaluate(
            ImmutableList.of(ModuleFileValue.KEY_FOR_ROOT_MODULE), evaluationContext);
    assertThat(result.hasError()).isTrue();
    assertThat(result.getError().toString()).contains("bad import label");
  }

  @Test
  public void testRootModule_moduleImport_bad_moduleAfterImport() throws Exception {
    scratch.overwriteFile(
        rootDirectory.getRelative("MODULE.bazel").getPathString(),
        "module_import('//java:MODULE.bazel.segment')");
    scratch.overwriteFile(rootDirectory.getRelative("java/BUILD").getPathString());
  ...
  @Test
  public void testRootModule_moduleImport_bad_repoNameCollision() throws Exception {
    scratch.overwriteFile(
        rootDirectory.getRelative("MODULE.bazel").getPathString(),
        "module(name='aaa')",
        "module_import('//java:MODULE.bazel.segment')",
  ...
  @Test
  public void testRootModule_moduleImport_bad_tryingToLeakBindings() throws Exception {
    scratch.overwriteFile(
        rootDirectory.getRelative("MODULE.bazel").getPathString(),
        "module(name='aaa')",
        "FOO_NAME = 'foo'",
  ...
  @Test
  public void testNonRootModuleCannotUseModuleImport() throws Exception {
